...
import com.horsehour.ml.model.LassoModel;
...
public class Lasso {
	...
	public int getMaxAllowedObservations(int maxNumFeatures) {
		...
		if (maxObservations > MAX_OBSERVATIONS_TO_FEATURES_RATIO * maxNumFeatures) {
			// maxObservations = MAX_OBSERVATIONS_TO_FEATURES_RATIO *
			// maxNumFeatures;
		}
	...
	private LassoModel getLassoFit(int maxAllowedFeaturesPerModel) {
		...
		/********************************
		 * Standardize features and target: Center the target and features (mean
		 * 0) and normalize their vectors to have the same standard deviation
		 */
		...
		for (int j = 0; j < numFeatures; j++) {
			double mean = MathLib.Data.mean(observations[j]);
			featureMeans[j] = mean;
			for (int i = 0; i < numObservations; i++) {
				observations[j][i] = (float) (factor * (observations[j][i] - mean));
			}
			featureStds[j] = Math.sqrt(MathLib.Matrix.innerProd(observations[j], observations[j]));

			observations[j] = MathLib.Matrix.multiply(observations[j], (float) (1.0D / featureStds[j]));
		}

		float targetMean = MathLib.Data.mean(targets);
		for (int i = 0; i < numObservations; i++) {
			targets[i] = factor * (targets[i] - targetMean);
		}
		float targetStd = (float) Math.sqrt(MathLib.Matrix.innerProd(targets, targets));
		targets = MathLib.Matrix.multiply(targets, 1.0F / targetStd);

		for (int j = 0; j < numFeatures; j++) {
			feature2residualCorrelations[j] = MathLib.Matrix.innerProd(targets, observations[j]);
		}

		double[][] feature2featureCorrelations = new double[numFeatures][maxAllowedFeaturesAlongPath];
		...
		LassoModel fit = new LassoModel(numberOfLambdas, maxAllowedFeaturesAlongPath, numFeatures);
		...
		for (int iteration = 1; iteration <= numberOfLambdas; iteration++) {
			...
			while (true) {
				...
				for (int k = 0; k < numFeatures; k++) {
						...
						for (int j = 0; j < numFeatures; j++) {
							...
							if (correlationCacheIndices[j] != 0) {
								feature2featureCorrelations[j][numberOfInputs
										- 1] = feature2featureCorrelations[k][correlationCacheIndices[j] - 1];
							} else {
								// Correlation of variable with itself if one
								if (j == k) {
									feature2featureCorrelations[j][numberOfInputs - 1] = 1.0;
								} else {
									feature2featureCorrelations[j][numberOfInputs - 1] = MathLib.Matrix
											.innerProd(observations[j], observations[k]);
								}
							}
						}
					...
					for (int j = 0; j < numFeatures; j++) {
						feature2residualCorrelations[j] -= feature2featureCorrelations[j][correlationCacheIndices[k]
								- 1] * delta;
					}
				}
				...
				do {
					...
					for (int l = 0; l < numberOfInputs; l++) {
						...
						for (int j = 0; j < numberOfInputs; j++) {
							feature2residualCorrelations[fit.indices[j]] -= feature2featureCorrelations[fit.indices[j]][correlationCacheIndices[k]
									- 1] * delta;
						}
					}
				...
				for (int j = 0; j < numFeatures; j++) {
					if (correlationCacheIndices[j] == 0) {
						feature2residualCorrelations[j] -= getDotProduct(denseActiveSet, feature2featureCorrelations[j],
								numberOfInputs);
					}
				}
			}
	...
	public double getDotProduct(double[] vector1, double[] vector2, int length) {
		double product = 0;
	...
	public LassoModel fit(int maxAllowedFeaturesPerModel) {
		LassoModel fit = getLassoFit(maxAllowedFeaturesPerModel);
		return fit;
	}
}