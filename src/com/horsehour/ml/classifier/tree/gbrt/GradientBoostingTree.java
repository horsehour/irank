/*
 * The stochastic gradient boosting method. yorkey: yangchadam AT gmail.com
 */
...
public class GradientBoostingTree {
	...
	GradientBoostingTree(double sample_size_ratio, double learning_rate, int tree_number,
	        int tree_min_nodes, int tree_depth) {
		/*
		 * Check the validity of numbers
		 */
		if (sample_size_ratio <= 0 || learning_rate <= 0 || tree_number < 1 || tree_min_nodes < 3
		        || tree_depth < 1) {
			System.out.println(String.format("Wrong parameters:  "
			        + "sample_size_ratio=%f, learning_rate=%f, tree_number=%d", sample_size_ratio,
			        learning_rate, tree_number));
	...
	/*
	 * fit a regression function using the Gradient Boosting Tree method. On
	 * success, return function; otherwise, return null.
	 */
	...
	/*
	 * The function is used to learn a rank function from data Input: x is
	 * preferred over y.
	 */
	public ResultFunction gradient_boosting_ranker(Vector<Vector<Double>> input_x,
		...
		if (feature_num != input_y.size() || feature_num == 0) {
			System.out
			        .println("The size of input_x should be the same as the size " + "of input_y");
		...
		while (iter_index < m_tree_number) {
			...
			// in the boosting ranker, randomly select half samples without
			// replacement in each
			// iteration
			RandomSample sampler = new RandomSample(feature_num, (int) (0.5 * feature_num));
			...
			for (int loop_index = 0; loop_index < feature_num; loop_index++) {
				h_value_x[loop_index] += m_learning_rate
				        * tree.get_single_value(input_x.get(loop_index));

				h_value_y[loop_index] += m_learning_rate
				        * tree.get_single_value(input_y.get(loop_index));
