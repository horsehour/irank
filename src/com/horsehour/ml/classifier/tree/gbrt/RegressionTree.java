...
package com.horsehour.ml.classifier.tree.gbrt;
...
public class RegressionTree {
	...
	private void print_each(Node current, String info, String kind) {
		if (current != null) {
			...
			System.out.println("Current node information: "
					+ String.format("(Level, side)=(%s,%s) with split information (%f, %d)", info, kind,
							current.m_node_value, current.m_feature_index));
	...
	/*
	 * The following class is used to define a split in the regression tree
	 * method
	 */
	...
	public class ComparatorFeature implements Comparator<Object> {
		public int compare(Object arg0, Object arg1) {
			...
			return data0.getx().compareTo(data1.getx());
		}
	}
	 ...
	 */
	private BestSplit get_optimal_split(Vector<Vector<Double>> feature_x, Vector<Double> obs_y) {
		...
		for (int loop_i = 0; loop_i < feature_dim; loop_i++) {
			...
			for (int loop_j = 0; loop_j < feature_num; loop_j++) {
				list_feature.add(new ListData(feature_x.get(loop_j).get(loop_i), obs_y.get(loop_j)));
			}
			...
			// = ||x||_2^2 - left_count * mean(x_l)^2 - right_count *
			// mean(x_r)^2
			// = constant - left_count * mean(x_l)^2 - right_count * mean(x_r)^2
			// Thus, we only need to check "- left_count * mean(x_l)^2 -
			// right_count * mean(x_r)^2"
			current_err = -1 * count_left * mean_left * mean_left - count_right * mean_right * mean_right;

			// current node value
			current_node_value = (list_feature.get(m_min_nodes).getx() + list_feature.get(m_min_nodes - 1).getx()) / 2;

			if (current_err < min_err && current_node_value != list_feature.get(m_min_nodes - 1).getx()) {
				split_index = loop_i;
			...
			for (int loop_j = m_min_nodes; loop_j <= feature_num - m_min_nodes - 1; loop_j++) {
				...
				current_err = -1 * count_left * mean_left * mean_left - count_right * mean_right * mean_right;
				// current node value
				current_node_value = (list_feature.get(loop_j + 1).getx() + fetched_data.getx()) / 2;

				if (current_err < min_err && (current_node_value != fetched_data.getx())) {
					split_index = loop_i;
					node_value = current_node_value;
					min_err = current_err;
				}
	...
	/*
	 * Split data into the left node and the right node based on the best
	 * splitting point.
	 */
	private SplitRes split_data(Vector<Vector<Double>> feature_x, Vector<Double> obs_y, BestSplit best_split) {
		...
		/*
		 * // used in debugging the program if (split_res.m_obs_left.size() <
		 * m_min_nodes || split_res.m_obs_right.size() < m_min_nodes) {
		 * get_optimal_split(feature_x, obs_y); System.exit(0); }
		 */
	 ...
	 */
	private Node tree_builder(Vector<Vector<Double>> feature_x, Vector<Double> obs_y) {
		...
		Node new_node = new Node(best_split.m_node_value, best_split.m_feature_index, split_data.m_left_value,
				split_data.m_right_value);

		if (m_root == null) {
			m_root = new_node;
			m_current_depth = 0;
			// append left and right side
			m_root.m_left_child = tree_builder(split_data.m_feature_left, split_data.m_obs_left); // left
			m_root.m_right_child = tree_builder(split_data.m_feature_right, split_data.m_obs_right); // right
		} else {
			// append left and right side
			new_node.m_left_child = tree_builder(split_data.m_feature_left, split_data.m_obs_left); // left
			new_node.m_right_child = tree_builder(split_data.m_feature_right, split_data.m_obs_right); // right
		}
		if (m_current_depth > 0)
			m_current_depth--;
		return new_node;
	...
	/*
	 * The min_node is minimum number of observations in the trees terminal
	 * nodes.
	 */
	public void build_regression_tree(Vector<Vector<Double>> feature_x, Vector<Double> obs_y) {
		int feature_num = feature_x.size();

		if (feature_num != obs_y.size() || feature_num == 0) {
			System.out.println("The number of features does not match" + " with the nunber of observations or "
