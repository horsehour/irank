 ...
 */
public class MDT implements SoftClassifier<double[]> {
	/**
	...
	public static class Trainer extends ClassifierTrainer<double[]> {
		...
		@Override
		public MDT train(double[][] x, int[] y) {
			return new MDT(null, x, y, width, numWidth, maxNodes, nodeSize, rule);
		}
	}
	...
	class Node {
		...
		public int predict(double[] x) {
			if (falseChild == null || (trueChild == null && splitFeature < dim))
				return output;
			...
			double[] lc = new double[numWidth];
			for (int k = 0; k < numWidth; k++) {
				int startId = k * numWidth;
				for (int t = 0; t < width; t++)
					lc[k] += x[startId + t] * weightFeatures[t];
			}
			int truePred = MathLib.getRank(lc, false)[0];
			return truePred;
		}
		...
		public int predict(double[] x, double[] posteriori) {
			...
			int index = 0;
			double sum = 0;
			int i = width * (splitFeature - dim);
			for (; index < width; i++) {
				sum += weightFeatures[index] * x[i];
				index++;
			}
	...
	class TrainNode implements Comparable<TrainNode> {
		...
		public boolean findBestSplit() {
			...
			if (mtry < p) {
				...
				List<SplitTask> tasks = new ArrayList<>(mtry + 1);
				for (int j = 0; j < mtry + 1; j++) {
		...
		public Node findBestSplit(int n, int[] count, int[] falseCount, double impurity, int j) {
			...
			/**
			 * Multivariate spliting for linear combination of features
			 */
			if (j >= dim) {
				double[] weight = new double[width * numWidth];
				for (int i = 0; i < n; i++) {
					int startId = y[i] * numWidth;
					for (int t = 0; t < width; t++)
						weight[t] += x[i][startId + t];
				}
				double sum = MathLib.Data.sum(weight);
				for (int k = 1; k < numWidth; k++) {
					int startId = k * numWidth;
					for (int t = 0; t < width; t++)
						weight[startId + t] = weight[t];
				}
				weight = MathLib.Matrix.multiply(weight, 1.0 / sum);

				double[][] lc = new double[n][numWidth];
				for (int i = 0; i < n; i++) {
					for (int k = 0; k < numWidth; k++) {
						int startId = k * numWidth;
						for (int t = 0; t < width; t++)
							lc[i][k] += x[i][startId + t] * weight[t];
					}
				}
				...
				for (int i = 0; i < n; i++) {
					if (samples[i] > 0) {
						int pred = MathLib.getRank(lc[i], false)[0];
						if (pred == y[i])
							trueCount[y[i]] += samples[i];
					}
				}
				...
				int falseLabel = Math.whichMax(falseCount);
				double gain = impurity - (double) fc / n * impurity(falseCount, fc);

				if (gain > splitNode.splitScore) {
					// new best split
					splitNode.splitFeature = j;
					splitNode.splitValue = -1;
					splitNode.splitScore = gain;
					splitNode.weightFeatures = weight;
					splitNode.trueChildOutput = -1;
					splitNode.falseChildOutput = falseLabel;
				}
			}
			return splitNode;
			...
			if (node.splitFeature >= dim) {
				...
				for (int i = 0; i < n; i++) {
					for (int k = 0; k < numWidth; k++) {
						int startId = k * numWidth;
						for (int t = 0; t < width; t++)
							lc[i][k] += x[i][startId + t] * node.weightFeatures[t];
					}
				}
	 ...
	 *            the maximum number of leaf nodes in the tree.
	 */
	public MDT(double[][] x, int[] y, int maxNodes) {
	 ...
	 *            the splitting rule.
	 */
	public MDT(double[][] x, int[] y, int maxNodes, SplitRule rule) {
	 ...
	 *            the splitting rule.
	 */
	public MDT(double[][] x, int[] y, int maxNodes, int nodeSize, SplitRule rule) {
	 ...
	 *            the maximum number of leaf nodes in the tree.
	 */
	public MDT(Attribute[] attributes, double[][] x, int[] y, int maxNodes) {
	 ...
	 *            the splitting rule.
	 */
	public MDT(Attribute[] attributes, double[][] x, int[] y, int maxNodes, SplitRule rule) {
	 ...
	 *            the splitting rule.
	 */
	public MDT(Attribute[] attributes, double[][] x, int[] y, int maxNodes, int nodeSize, SplitRule rule) {
		this(attributes, x, y, maxNodes, nodeSize, x[0].length, rule, null, null);
	}

	public MDT(Attribute[] attributes, double[][] x, int[] y, int width, int numWidth, int maxNodes, int nodeSize,
			SplitRule rule) {
		this(attributes, x, y, width, numWidth, maxNodes, nodeSize, x[0].length, rule, null, null);
	}
	 ...
	 */
	public MDT(Attribute[] attributes, double[][] x, int[] y, int maxNodes, int nodeSize, int mtry, SplitRule rule,
			int[] samples, int[][] order) {
		this(attributes, x, y, -1, -1, maxNodes, nodeSize, mtry, rule, samples, order);
	}

	public MDT(Attribute[] attributes, double[][] x, int[] y, int width, int numWidth, int maxNodes, int nodeSize,
			int mtry, SplitRule rule, int[] samples, int[][] order) {
		...
		this.numWidth = (numWidth < 0) ? 0 : numWidth;
		this.dim = x[0].length;
		...
		this.maxNodes = maxNodes;
		this.rule = rule;
		importance = new double[attributes.length + numWidth];
		...
		for (int leaves = 1; leaves < this.maxNodes; leaves++) {
			...
			node.split(nextSplits); // Split the parent node into two children
									// nodes
		}
	}
	...
	private void printSubtree(Node node, StringBuffer sb) {
		if (node.trueChild != null)
			printTree(node.trueChild, true, "", sb);

		printNodeValue(node, sb);
		if (node.falseChild != null)
			printTree(node.falseChild, false, "", sb);
	}

	private void printNodeValue(Node node, StringBuffer sb) {
		if (node.falseChild == null && node.trueChild == null)
			sb.append(node.output + "\n");
		else if (node.splitFeature < dim) {
			sb.append("X" + node.splitFeature + " > " + String.format("%.2f", node.splitValue) + "");
		} else {
			int k = width * (node.splitFeature - dim);
			int index = 0;
			StringBuffer buffer = new StringBuffer();
			for (; index < width; k++) {
				buffer.append(String.format("%.2f", node.weightFeatures[index]) + " * X" + k + " + ");
				index++;
			}
			String str = buffer.toString();
			sb.append(str.substring(0, str.length() - 2).trim() + " > " + String.format("%.2f", node.splitValue));
		}
		sb.append("\n");

	}

	private void printTree(Node node, boolean isRight, String indent, StringBuffer sb) {
		if (node.trueChild != null)
			printTree(node.trueChild, true, indent + (isRight ? "        " : " |      "), sb);
		...
		printNodeValue(node, sb);
		if (node.falseChild != null) {
			printTree(node.falseChild, false, indent + (isRight ? " |      " : "        "), sb);
		}
	}
