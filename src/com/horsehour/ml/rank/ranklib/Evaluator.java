...
import com.horsehour.ml.rank.ranklib.Ranker;
...
public class Evaluator {
	...
	public Evaluator(RANKER_TYPE rt, String trainMetric, String testMetric) {
		this.type = rt;
		trainScorer = mFact.createScorer(trainMetric);
		testScorer = mFact.createScorer(testMetric);
		if (qrelFile.compareTo("") != 0) {
	 ...
	 */
	public void evaluate(String trainFile, String valiFile, String testFile,
		...
		if (test != null) {
			...
			System.out.println(testScorer.name() + " on test data: "
	 ...
	 * the samples for training the rest for testing.
	 * 
	 * @param sampleFile
	...
	public void evaluate(String sampleFile, String featureDefFile, int nFold) {
		...
		int[] features = prepareCV(sampleFile, featureDefFile, nFold,
		        normalize, trainingData, testData);
	...
	public void test(String modelFile, String testFile, boolean printIndividual) {
		...
		for (int i = 0; i < test.size(); i++) {
			...
			if (printIndividual)
				System.out.println(testScorer.name() + "   " + l.getID()
				        + "   " + SimpleMath.round(score, 4));
			rankScore += score;
		}
		rankScore /= test.size();
		if (printIndividual)
			System.out.println(testScorer.name() + "   all   "
			        + SimpleMath.round(rankScore, 4));
		else
			System.out.println(testScorer.name() + " on test data: "
			        + SimpleMath.round(rankScore, 4));
	...
	private int[] prepareCV(String sampleFile, String featureDefFile,
	        int nFold, boolean normalize, List<List<RankList>> trainingData,
		...
		if (newFeatureFile.compareTo("") != 0) {
			...
			List<String> descriptions = FileUtils.readLine(newFeatureFile,
			        "ASCII");
	...
	private int[] applyNewFeatures(List<RankList> samples, int[] features) {
		...
		for (int i = 0; i < samples.size(); i++) {
			...
			for (int j = 0; j < rl.size(); j++) {
				...
				for (int k = 0; k < lcList.size(); k++)
					addedFeatures[k] = lcList.get(k).compute(
					        p.getExternalFeatureVector());
	...
	public static void main_0(String[] args) {
		...
		String[] rn = new String[] { "MART", "RankNet", "RankBoost", "AdaRank",
		        "Coordinate Ascent", "LambdaRank", "LambdaMART", "ListNet",
		        "Random Forests" };

		RANKER_TYPE[] rt = new RANKER_TYPE[] { RANKER_TYPE.MART,
		...
		if (args.length < 2) {
			...
			System.out
			        .println("\t-ranker <type>\t\tSpecify which ranking algorithm to use");
			System.out
			        .println("\t\t\t\t0: MART (gradient boosted regression tree)");
			...
			System.out
			        .println("\t[ -feature <file> ]\tFeature description file: list features to be considered by the learner, each on a separate line");
			System.out
			        .println("\t\t\t\tIf not specified, all features will be used.");
			// System.out.println("\t[ -metric2t <metric> ]\tMetric to optimize on the training data. Supported: MAP, NDCG@k, DCG@k, P@k, RR@k, BEST@k, ERR@k (default="
			// + trainMetric + ")");
			System.out
			        .println("\t[ -metric2t <metric> ]\tMetric to optimize on the training data. Supported: MAP, NDCG@k, DCG@k, P@k, RR@k, ERR@k (default="
			                + trainMetric + ")");
			System.out
			        .println("\t[ -metric2T <metric> ]\tMetric to evaluate on the test data (default to the same as specified for -metric2t)");
			System.out
			        .println("\t[ -gmax <label> ]\tHighest judged relevance label. It affects the calculation of ERR (default="
			...
			System.out
			        .println("\t[ -test <file> ]\tSpecify if you want to evaluate the trained model on this data (default=unspecified)");
			System.out
			        .println("\t[ -validate <file> ]\tSpecify if you want to tune your system on the validation data (default=unspecified)");
			System.out
			        .println("\t\t\t\tIf specified, the final model will be the one that performs best on the validation data");
			System.out
			        .println("\t[ -tvs <x \\in [0..1]> ]\tSet train-validation split to be (x)(1.0-x)");
			System.out
			        .println("\t[ -tts <x \\in [0..1]> ]\tSet train-test split to be (x)(1.0-x). -tts will override -tvs");
			System.out
			        .println("\t[ -kcv <k> ]\t\tSpecify if you want to perform k-fold cross validation using ONLY the specified training data (default=NoCV)");

			System.out
			        .println("\t[ -norm <method>]\tNormalize feature vectors (default=no-normalization). Method can be:");
			System.out
			        .println("\t\t\t\tsum: normalize each feature by the sum of all its values");
			System.out
			        .println("\t\t\t\tzscore: normalize each feature by its mean/standard deviation");

			System.out
			        .println("\t[ -save <model> ]\tSave the learned model to the specified file (default=not-save)");

			System.out
			        .println("\t[ -silent ]\t\tDo not print progress messages (which are printed by default)");

			System.out.println("");
			System.out.println("    [-] RankNet-specific parameters");
			System.out
			        .println("\t[ -epoch <T> ]\t\tThe number of epochs to train (default="
			                + RankNet.nIteration + ")");
			System.out
			        .println("\t[ -layer <layer> ]\tThe number of hidden layers (default="
			                + RankNet.nHiddenLayer + ")");
			System.out
			        .println("\t[ -node <node> ]\tThe number of hidden nodes per layer (default="
			                + RankNet.nHiddenNodePerLayer + ")");
			System.out.println("\t[ -lr <rate> ]\t\tLearning rate (default="
			        + (new DecimalFormat("###.########"))
			                .format(RankNet.learningRate) + ")");

			System.out.println("");
			System.out.println("    [-] RankBoost-specific parameters");
			System.out
			        .println("\t[ -round <T> ]\t\tThe number of rounds to train (default="
			                + RankBoost.nIteration + ")");
			System.out
			        .println("\t[ -tc <k> ]\t\tNumber of threshold candidates to search. -1 to use all feature values (default="
			...
			System.out
			        .println("\t[ -round <T> ]\t\tThe number of rounds to train (default="
			                + AdaRank.nIteration + ")");
			System.out
			        .println("\t[ -noeq ]\t\tTrain without enqueuing too-strong features (default=unspecified)");
			System.out
			        .println("\t[ -tolerance <t> ]\tTolerance between two consecutive rounds of learning (default="
			                + AdaRank.tolerance + ")");
			System.out
			        .println("\t[ -max <times> ]\tThe maximum number of times can a feature be consecutively selected without changing performance (default="
			                + AdaRank.maxSelCount + ")");

			System.out.println("");
			System.out.println("    [-] Coordinate Ascent-specific parameters");
			System.out
			        .println("\t[ -r <k> ]\t\tThe number of random restarts (default="
			                + CoorAscent.nRestart + ")");
			System.out
			        .println("\t[ -i <iteration> ]\tThe number of iterations to search in each dimension (default="
			                + CoorAscent.nMaxIteration + ")");
			System.out
			        .println("\t[ -tolerance <t> ]\tPerformance tolerance between two solutions (default="
			                + CoorAscent.tolerance + ")");
			System.out
			        .println("\t[ -reg <slack> ]\tRegularization parameter (default=no-regularization)");

			System.out.println("");
			System.out
			        .println("    [-] {MART, LambdaMART}-specific parameters");
			System.out.println("\t[ -tree <t> ]\t\tNumber of trees (default="
			        + LambdaMART.nTrees + ")");
			System.out
			        .println("\t[ -leaf <l> ]\t\tNumber of leaves for each tree (default="
			                + LambdaMART.nTreeLeaves + ")");
			System.out
			        .println("\t[ -shrinkage <factor> ]\tShrinkage, or learning rate (default="
			                + LambdaMART.learningRate + ")");
			System.out
			        .println("\t[ -tc <k> ]\t\tNumber of threshold candidates for tree spliting. -1 to use all feature values (default="
			                + LambdaMART.nThreshold + ")");
			System.out
			        .println("\t[ -mls <n> ]\t\tMin leaf support -- minimum #samples each leaf has to contain (default="
			                + LambdaMART.minLeafSupport + ")");
			System.out
			        .println("\t[ -estop <e> ]\t\tStop early when no improvement is observed on validaton data in e consecutive rounds (default="
			...
			System.out.println("\t[ -lr <rate> ]\t\tLearning rate (default="
			        + (new DecimalFormat("###.########"))
			                .format(ListNet.learningRate) + ")");

			System.out.println("");
			System.out.println("    [-] Random Forests-specific parameters");
			System.out.println("\t[ -bag <r> ]\t\tNumber of bags (default="
			        + RFRanker.nBag + ")");
			System.out
			        .println("\t[ -srate <r> ]\t\tSub-sampling rate (default="
			                + RFRanker.subSamplingRate + ")");
			System.out
			        .println("\t[ -frate <r> ]\t\tFeature sampling rate (default="
			                + RFRanker.featureSamplingRate + ")");
			int type = (RFRanker.rType.ordinal() - RANKER_TYPE.MART.ordinal());
			System.out.println("\t[ -rtype <type> ]\tRanker to bag (default="
			        + type + ", i.e. " + rn[type] + ")");
			System.out
			        .println("\t[ -tree <t> ]\t\tNumber of trees in each bag (default="
			                + RFRanker.nTrees + ")");
			System.out
			        .println("\t[ -leaf <l> ]\t\tNumber of leaves for each tree (default="
			...
			System.out
			        .println("\t-test <file>\t\tTest data to evaluate the model (specify either this or -rank but not both)");
			System.out
			        .println("\t-rank <file>\t\tRank the samples in the specified file (specify either this or -test but not both)");
			System.out
			        .println("\t[ -metric2T <metric> ]\tMetric to evaluate on the test data (default="
			                + trainMetric + ")");
			System.out
			        .println("\t[ -gmax <label> ]\tHighest judged relevance label. It affects the calculation of ERR (default="
			                + (int) SimpleMath.logBase2(ERRScorer.MAX)
			                + ", i.e. 5-point scale {0,1,2,3,4})");
			System.out
			        .println("\t[ -score <file>]\tStore ranker's score for each object being ranked (has to be used with -rank)");
			// System.out.println("\t[ -qrel <file> ]\tTREC-style relevance judgment file. It only affects MAP and NDCG (default=unspecified)");
			System.out
			        .println("\t[ -idv ]\t\tPrint model performance (in test metric) on individual ranked lists (has to be used with -test)");
			System.out
			        .println("\t[ -norm ]\t\tNormalize feature vectors (similar to -norm for training/tuning)");
		...
		Evaluator e = new Evaluator(rt[rankerType], trainMetric, testMetric);
		if (trainFile.compareTo("") != 0) {
			...
			if (foldCV != -1) {
				...
				else if (ttSplit > 0.0)// choose to split training data into
									   // train and test
					System.out.println("Train-Test split: " + ttSplit);

				if (validationFile.compareTo("") != 0)// the user has specified
													  // the validation set
			...
			System.out.println("Ranking method:\t" + rn[rankerType]);
			if (featureDescriptionFile.compareTo("") != 0)
				System.out.println("Feature description file:\t"
			...
			if (trainMetric.toUpperCase().startsWith("ERR")
			        || testMetric.toUpperCase().startsWith("ERR"))
				System.out.println("Highest relevance label (to compute ERR): "
				        + (int) SimpleMath.logBase2(ERRScorer.MAX));
			if (qrelFile.compareTo("") != 0)
				System.out
			...
			System.out.println("");
			System.out.println("[+] " + rn[rankerType] + "'s Parameters:");

			RankerFactory rf = new RankerFactory();

			rf.createRanker(rt[rankerType]).printParameters();
			System.out.println("");
			...
			if (foldCV != -1)
				...
				if (ttSplit > 0.0)// we should use a held-out portion of the
					...
					e.evaluate(trainFile, validationFile,
					        featureDescriptionFile, ttSplit);
		...
		} else // scenario: test a saved model
		{
			...
			if (rankFile.compareTo("") != 0) {
				...
				// This is *ONLY* for my personal use. It is *NOT* exposed via
				// cmd-line
				// It will evaluate the input ranking (without being reranked by
				// any model) using any measure specified via metric2T
				else
					e.test(testFile);
			}
		}
	...
	public static void main(String[] args) {
		String train = "F:/Research/Data/MQ2008/Fold5/train.txt";
		String vali = "F:/Research/Data/MQ2008/Fold5/vali.txt";
		String test = "F:/Research/Data/MQ2008/Fold5/test.txt";

		String trainMetric = "NDCG@1";
		String testMetric = "NDCG@1";
		...
		Evaluator.modelFile = "C:/Users/Dell/Desktop/model.txt";
		Evaluator.normalize = false;
		Evaluator.letor = true;

		MyThreadPool.init(Runtime.getRuntime().availableProcessors());

		Evaluator eva = new Evaluator(RANKER_TYPE.ADARANK, trainMetric,
		        testMetric);
		eva.evaluate(train, vali, test, "");
