...
public class Neuron {
	...
	protected double output;// sigmoid(wsum) (range from 0.0 to 1.0): output for
							// the current input
	protected List<Double> outputs = null;// 针对每一个输入向量在后向传播过程中存储的输出，其维数恰好为输入向量的个数
	...
	public void computeOutput() {
		...
		output = (double) tfunc.compute(wsum);// using the specified transfer
											  // function to compute the output
	}

	// 计算第i个输入特征向量在传播过程中，由当前神经元保存的输出值，放到outputs中，因此其维数是输入特征向量的数目
	// 比如与某个query关联的特征向量的数目为N，那么就需要计算N个输出值，则outputs.size()==N
	public void computeOutput(int i) {
		Synapse s = null;
		double wsum = 0.0;
		for (int j = 0; j < inLinks.size(); j++) {
			s = inLinks.get(j);
			wsum += s.getSource().getOutput(i) * s.getWeight();// TODO:参考neuroph框架，定义InputFunction,WeightedFunction,为neuron定义NetInput
		}
		output = (double) tfunc.compute(wsum);// using the specified transfer
											  // function to compute the output
		outputs.add(output);
	...
	/**
	 * Compute delta for neurons in the output layer. ONLY for output neurons
	 * 
	 * @param targetValue
	 */
	public void computeDelta(PropParameter param) {
		/*
		 * double pij = (double) (1.0 / (1.0 +
		 * Math.exp(-(prev_output-output)))); prev_delta = (targetValue-pij) *
		 * tfunc.computeDerivative(prev_output); delta = (targetValue-pij) *
		 * tfunc.computeDerivative(output);
		 */
		...
		for (int k = 0; k < pairMap[current].length; k++) {
			...
			if (param.pairWeight == null)// RankNet, no pair-weight needed
			{
				weight = 1;
				// pij := 1-P
				pij = (double) (1.0 / (1.0 + Math.exp(outputs.get(current)
				        - outputs.get(pairId))));// this is in fact not "pij",
												 // but "targetValue-pij": 1 -
												 // 1/(1+e^{-o_ij})
			} else// LambdaRank
			{
				weight = param.pairWeight[current][k];
				pij = (double) (param.targetValue[current][k] - 1.0 / (1.0 + Math
				        .exp(-(outputs.get(current) - outputs.get(pairId)))));
			}

			double lambda = weight * pij;
			delta_i += lambda;
			// deltas_j[k] = lambda *
			// tfunc.computeDerivative(outputs.get(pairId));

			derivation = outputs.get(pairId) * (1 - outputs.get(pairId));
			deltas_j[k] = lambda * derivation;
		...
		delta_i *= derivation;
		// (delta_i * input_i) - (sum_{delta_j} * input_j) is the *negative* of
		// the gradient, which is the amount of weight should be added to the
		// current weight
	...
	public void updateDelta(PropParameter param) {
		/*
		 * double errorSum = 0.0; Synapse s = null; for(int
		 * i=0;i<outLinks.size();i++) { s = outLinks.get(i); errorSum +=
		 * (s.getTarget().getPrevDelta()-s.getTarget().getDelta()) *
		 * s.getWeight(); } prev_delta = errorSum *
		 * tfunc.computeDerivative(prev_output); delta = errorSum *
		 * tfunc.computeDerivative(output);
		 */
		int[][] pairMap = param.pairMap;
		...
		for (int k = 0; k < pairMap[current].length; k++) {
			...
			if (k == 0) {
				// delta_i *= weight *
				// tfunc.computeDerivative(outputs.get(current));
				derivation = outputs.get(current) * (1 - outputs.get(current));
				delta_i *= weight * derivation;
			}
			derivation = outputs.get(pairId) * (1 - outputs.get(pairId));
			deltas_j[k] = errorSum * weight * derivation;
			// deltas_j[k] = errorSum * weight *
			// tfunc.computeDerivative(outputs.get(pairId));
		}
	}
	...
	public void updateWeight(PropParameter param) {
		...
		for (int k = 0; k < inLinks.size(); k++) {
			...
			for (int l = 0; l < deltas_j.length; l++)
				sum_j += deltas_j[l]
				        * s.getSource().getOutput(
				                param.pairMap[param.current][l]);
			double dw = learningRate
			        * (delta_i * s.getSource().getOutput(param.current) - sum_j);
