...
/**
 * @author vdang This class implements MAP (Mean Average Precision)
 */
public class APScorer extends MetricScorer {
	// This class computes MAP from the *WHOLE* ranked list. "K" will be
	// completely ignored.
	// The reason is, if you want MAP@10, you really should be using NDCG@10 or
	// ERR@10 instead.
	...
	public void loadExternalRelevanceJudgment(String qrelFile) {
		...
		try {
			...
			relDocCount.put(lastQID, rdCount);
			in.close();
			System.out.println("Relevance judgment file loaded. [#q="
			        + relDocCount.keySet().size() + "]");
		} catch (Exception ex) {
			System.out
			        .println("Error in APScorer::loadExternalRelevanceJudgment(): "
			                + ex.toString());
		}
	}

	/**
	 * Compute Average Precision (AP) of the list. AP of a list is the average
	 * of precision evaluated at ranks where a relevant document is observed.
	 * 
	...
	public double score(RankList rl) {
		...
		if (relDocCount != null) {
			...
			// no qrel-file specified, we can only use the #relevant-docs in the
			// training file
			rdCount = count;
	...
	public double[][] swapChange(RankList rl) {
		// NOTE: Compute swap-change *IGNORING* K (consider the entire ranked
		// list)
		...
		for (int i = 0; i < rl.size() - 1; i++) {
			for (int j = i + 1; j < rl.size(); j++) {
				...
				if (labels[i] != labels[j]) {
					...
					change += ((double) ((relCount[i] + diff) * labels[j] - relCount[i]
					        * labels[i]))
					        / (i + 1);
					...
					// It is equivalent to: change +=
					// ((double)(relCount[j]*labels[i] - relCount[j]*labels[j]))
					// / (j+1);
				}
