...
/**
 * 
 * @author Van Dang Expected Reciprocal Rank
 */
public class ERRScorer extends MetricScorer {

	public static double MAX = 16;// by default, we assume the relevance scale
								  // of {0, 1, 2, 3, 4} => g_max = 4 => 2^g_max
								  // = 16
	...
	/**
	 * Compute ERR at k. NDCG(k) = DCG(k) / DCG_{perfect}(k). Note that the
	...
	public double[][] swapChange(RankList rl) {
		...
		double[] np = new double[rl.size()];// p[i] = (1 - p[0])(1 -
											// p[1])...(1-p[i-1])
		double p = 1.0;
		// for(int i=0;i<rl.size();i++)//ignore K, compute changes from the
		// entire ranked list
		...
		for (int i = 0; i < size; i++) {
			...
			for (int j = i + 1; j < rl.size(); j++) {
				if (labels[i] == labels[j])
					...
					change += (np[j - 1] * (1.0 - R[j]) * R[i] / (1.0 - R[i]) - np[j - 1]
					        * R[j])
					        / (j + 1);
				}
