...
public class RFRanker extends Ranker {
	...
	public static float subSamplingRate = 1.0f;// sampling of samples (*WITH*
											   // replacement)
	public static float featureSamplingRate = 0.3f;// sampling of features
												   // (*WITHOUT* replacement)
	// [b] what to do in each bag
	public static RANKER_TYPE rType = RANKER_TYPE.MART;// which algorithm to bag
	public static int nTrees = 1;// how many trees in each bag. If nTree > 1 ==>
								 // each bag will contain an ensemble of
								 // gradient boosted trees.
	public static int nTreeLeaves = 100;
	public static float learningRate = 0.1F;// or shrinkage. *ONLY* matters if
											// nTrees > 1.
	public static int nThreshold = 256;
	public static int minLeafSupport = 1;

	// Variables
	protected Ensemble[] ensembles = null;// bag of ensembles, each can be a
										  // single tree or an ensemble of
										  // gradient boosted trees
	...
	public void init() {
		...
		LambdaMART.nRoundToStopEarly = -1;// no early-stopping since we're doing
										  // bagging
	...
	public void learn() {
		...
		PRINTLN("------------------------------------");
		PRINTLN(new int[] { 9, 9, 11 }, new String[] { "bag",
		        scorer.name() + "-B", scorer.name() + "-OOB" });
		PRINTLN("------------------------------------");
		// start the bagging process
		for (int i = 0; i < nBag; i++) {
			System.gc();
			Sampler sp = new Sampler();
			// create a "bag" of samples by random sampling from the training
			// set
			...
			Ranker.verbose = false;// turn of the progress messages from
			...
			PRINTLN(new int[] { 9, 9 }, new String[] { "b[" + (i + 1) + "]",
			        SimpleMath.round(r.getScoreOnTrainingData(), 4) + "" });
