...
public class RegressionTree {
	...
	// Parameters
	protected int nodes = 10;// -1 for unlimited number of nodes (the size of
							 // the tree will then be controlled *ONLY* by
							 // minLeafSupport)
	...
	public RegressionTree(int nLeaves, DataPoint[] trainingSamples,
	        float[] labels, FeatureHistogram hist, int minLeafSupport) {
		this.nodes = nLeaves;
	...
	public void fit() {
		...
		int taken = 0;
		while ((nodes == -1 || taken + queue.size() < nodes)
		        && queue.size() > 0) {
			Split leaf = queue.get(0);
			queue.remove(0);

			Split s = split(leaf);
			if (s == null)// unsplitable (i.e. variance(s)==0; or after-split
						  // variance is higher than before; or #samples <
						  // minLeafSupport)
				taken++;
			else {
	...
	/**
	 * Find the best <feature, threshold> that split the set of samples into two
	 * subsets with the smallest S (mean squared error): S = sum_{samples put to
	 * the left}^{(label - muLeft)^2} + sum_{samples put to the right}^{(label -
	 * muRight)^2} and split the input tree node using this <feature, threshold>
	 * pair.
	 * 
	 * @param
	 * @return
	...
	protected Split split(Split s) {
		...
		return h.findBestSplit(s, trainingSamples, trainingLabels,
		        minLeafSupport);
	}
	...
	/**
	 * Clear samples associated with each leaves (when they are no longer
	 * necessary) in order to save memory
	 */
