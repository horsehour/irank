...
public class ListNet extends RankNet {
	...
	public static int nHiddenLayer = 0;// FIXED, it doesn't work with hidden
									   // layer
	...
	protected void backPropagate(float[] labels)// labels-ground truth labels
	{
		// back-propagate
	...
	public void learn() {
		...
		PRINTLN(new int[] { 7, 14, 9, 9 }, new String[] { "#epoch",
		        "C.E. Loss", scorer.name() + "-T", scorer.name() + "-V" });
		PRINTLN("--------------------------------------------------");
		// Loss function: Cross Entropy Loss(C.E. Loss)
		for (int i = 1; i <= nIteration; i++) {
			for (int j = 0; j < samples.size(); j++) {
				float[] labels = feedForward(samples.get(j));// labels-ground
															 // truth labels
			...
			estimateLoss();
			PRINT(new int[] { 7, 14 },
			        new String[] { i + "", SimpleMath.round(error, 6) + "" });
			if (i % 1 == 0) {
	...
	public String model() {
		...
		output += "0\n";// [# hidden layers, *ALWAYS* 0 since we're using linear
						// net]
