 ...
 * @author vdang
 * 
 *         This class implements RankBoost. Y. Freund, R. Iyer, R. Schapire, and
 *         Y. Singer. An efficient boosting algorithm for combining preferences.
...
public class RankBoost extends Ranker {
	...
	protected double[][][] sweight = null;// sample weight D(x_0, x_1) -- the
										  // weight of x_1 ranked above x_2
	protected double[][] potential = null;// pi(x)
	protected List<List<int[]>> sortedSamples = new ArrayList<List<int[]>>();
	protected double[][] thresholds = null;// candidate values for weak rankers'
										   // threshold, selected from feature
										   // values
	protected int[][] tSortedIdx = null;// sorted (descend) index for
										// @thresholds
	protected Hashtable<Integer, Integer> usedFeatures = new Hashtable<Integer, Integer>();

	protected List<RBWeakRanker> wRankers = null;// best weak rankers at each
												 // round
	...
	/**
	 * compute the potential (pi) based on the current sample (pair) weight
	 * distribution D_t.
	 */
	...
	/**
	 * Find the <feature, threshold> that maximize r (which will approximately
	 * minimize the exponential error on the training data). Create the weak
	 * ranker h_t from this pair <feature, threshold>. h_t(p) > threshold =>
	 * h_t(p)=1; h_t(p)=0 otherwise.
	 * 
	 * @return The learned weak ranker. The value of <i>current_r</i> is also
	 *         updated to be the best r observed.
	 */
	private RBWeakRanker learnWeakRanker() {
		...
		for (int i = 0; i < features.length; i++) {
			List<int[]> sSortedIndex = sortedSamples.get(i);// samples sorted
															// (descending) by
															// the current
															// feature
			int[] idx = tSortedIdx[i];// candidate thresholds for the current
									  // features
			int[] last = new int[samples.size()];// the last "touched" (and
												 // taken) position in each
												 // sample rank list
		...
		R_t = Z_t * maxR;// save it so we won't have to re-compute when we need
						 // it
	...
	public void init() {
		...
		// for each (true) ranked list, we only care about correctly ranked pair
		// (e.g. L={1,2,3} => <1,2>, <1,3>, <2,3>)
		// count the number of correctly ranked pairs from sample ranked list
		totalCorrectPairs = 0;
		for (int i = 0; i < samples.size(); i++) {
			samples.set(i, samples.get(i).getCorrectRanking());// make sure the
															   // training
															   // samples are in
															   // correct
															   // ranking
			...
			for (int j = 0; j < rl.size() - 1; j++)
				for (int k = rl.size() - 1; k >= j + 1
				        && rl.get(j).getLabel() > rl.get(k).getLabel(); k--)
		...
		if (nThreshold <= 0) {
			// create a table of candidate thresholds (for each feature) for
			// weak rankers (they are just all possible feature values)
			int count = 0;
			for (int i = 0; i < samples.size(); i++)
			...
			for (int i = 0; i < samples.size(); i++) {
				...
				for (int j = 0; j < rl.size(); j++) {
					for (int k = 0; k < features.length; k++)
						thresholds[k][c] = rl.get(j).getFeatureValue(
						        features[k]);
					c++;
				}
			}
		...
		// sort this table with respect to each feature (each row of the matrix
		// @thresholds)
		tSortedIdx = new int[features.length][];
		for (int i = 0; i < features.length; i++)
			tSortedIdx[i] = MergeSorter.sort(thresholds[i], false);

		// now create a sorted lists of every samples ranked list with respect
		// to each feature
		// e.g. Feature f_i <==> all sample ranked list is now ranked with
		// respect to f_i
		for (int i = 0; i < features.length; i++) {
	...
	public void learn() {
		...
		for (int t = 1; t <= nIteration; t++) {
			...
			double alpha_t = (double) (0.5 * SimpleMath.ln((Z_t + R_t)
			        / (Z_t - R_t)));// @current_r is computed in
									// learnWeakRanker()
			...
			for (int i = 0; i < samples.size(); i++) {
				...
				for (int j = 0; j < rl.size() - 1; j++) {
					...
					for (int k = j + 1; k < rl.size(); k++) {
						...
						// so if our h_t does so, decrease the weight of this
						// pair
						// otherwise, increase its weight
						D_t[j][k] = (double) (sweight[i][j][k] * Math
						        .exp(alpha_t
						                * (wr.score(rl.get(k)) - wr.score(rl
						                        .get(j)))));
						Z_t += D_t[j][k];
					}
				}
			...
			PRINT(new int[] { 7, 8, 9, 9 },
			        new String[] { t + "", wr.getFid() + "",
			                SimpleMath.round(wr.getThreshold(), 4) + "",
			                SimpleMath.round(R_t, 4) + "" });
			if (t % 1 == 0) {
				PRINT(new int[] { 9 },
				        new String[] { SimpleMath.round(
				                scorer.score(rank(samples)), 4)
				                + "" });
			...
			// System.out.println("Z_t = " + Z + "\tr = " + current_r + "\t" +
			// Math.sqrt(1.0 - current_r*current_r));
	...
	public String toString() {
		...
		for (int i = 0; i < wRankers.size(); i++)
			output += wRankers.get(i).toString() + ":" + rWeight.get(i)
			        + ((i == rWeight.size() - 1) ? "" : " ");
		return output;
	}
	...
	public void load(String fn) {
		try {
			...
			if (idx != -1)// remove description at the end of the line (if any)
				content = content.substring(0, idx).trim();// remove the comment
														   // part at the end of
														   // the line
