...
public class NDCGScorer extends MetricScorer {
	...
	public void loadExternalRelevanceJudgment(String qrelFile) {
		...
		try {
			...
			System.out.println("Relevance judgment file loaded. [#q="
			        + idealGains.keySet().size() + "]");
		} catch (Exception ex) {
			System.out
			        .println("Error in NDCGScorer::loadExternalRelevanceJudgment(): "
			                + ex.toString());
		}
	}
	 ...
	 * "perfect ranking" must be computed based on the whole list, not just
	 * top-k portion of the list.
	 */
	public double score(RankList rl) {
		List<Integer> rel = new ArrayList<Integer>();// ground truth label
	...
	private double getDCG(List<Integer> rel, int k) {
		...
		for (int i = 1; i <= size; i++) {
			dcg += (Math.pow(2.0, rel.get(i - 1)) - 1.0)
			        / SimpleMath.logBase2(i + 1);
		}
		return dcg;
	}
	...
	private double getIdealDCG(List<Integer> rel, int k) {
		...
		double dcg = 0.0;
		for (int i = 1; i <= size; i++) {
			dcg += (Math.pow(2.0, rel.get(idx[i - 1])) - 1.0)
			        / SimpleMath.logBase2(i + 1);
		}
		return dcg;
	}
	...
	public double[][] swapChange(RankList rl) {
		...
		// double d2 = getIdealDCG(rel, rl.size());//ignore K, compute changes
		// from the entire ranked list
		...
		// for(int i=0;i<rl.size()-1;i++)//ignore K, compute changes from the
		// entire ranked list
		{
			int p1 = i + 1;
			for (int j = i + 1; j < rl.size(); j++) {
				if (d2 > 0) {
					int p2 = j + 1;
					changes[j][i] = changes[i][j] = (1.0 / SimpleMath
					        .logBase2(p1 + 1) - 1.0 / SimpleMath
					        .logBase2(p2 + 1))
					        * (Math.pow(2.0, rel.get(i)) - Math.pow(2.0,
					                rel.get(j))) / d2;
				}
			}
		}
