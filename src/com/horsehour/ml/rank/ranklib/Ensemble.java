...
public class Ensemble {
	...
	public Ensemble(String xmlRep) {
		try {
			...
			for (int i = 0; i < nl.getLength(); i++) {
				...
				float weight = Float.parseFloat(n.getAttributes()
				        .getNamedItem("weight").getNodeValue().toString());
				// add it to the ensemble
				trees.add(new RegressionTree(root));
				weights.add(weight);
			}
		} catch (Exception ex) {
			System.out.println("Error in Emsemble(xmlRepresentation): "
			        + ex.toString());
		}
	...
	private Split create(Node n) {
		Split s = null;
		if (n.getFirstChild().getNodeName().compareToIgnoreCase("feature") == 0)// this
																				// is
																				// a
																				// split
		{
			NodeList nl = n.getChildNodes();
			int fid = Integer.parseInt(nl.item(0).getFirstChild()
			        .getNodeValue().toString().trim());// <feature>
			float threshold = Float.parseFloat(nl.item(1).getFirstChild()
			        .getNodeValue().toString().trim());// <threshold>
			s = new Split(fid, threshold, 0);
			s.setLeft(create(nl.item(2)));
			s.setRight(create(nl.item(3)));
		} else// this is a stump
		{
			float output = Float.parseFloat(n.getFirstChild().getFirstChild()
