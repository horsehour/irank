 ...
 * @author vdang
 * 
 *         This class implements RankNet. C.J.C. Burges, T. Shaked, E. Renshaw,
 *         A. Lazier, M. Deeds, N. Hamilton and G. Hullender. Learning to rank
 *         using gradient descent. In Proc. of ICML, pages 89-96, 2005.
 */
public class RankNet extends Ranker {
	...
	protected void setInputOutput(int nInput, int nOutput, int nType) {
		inputLayer = new Layer(nInput + 1, nType);// plus the "bias" (output
												  // threshold)
	...
	protected void wire() {
		...
		for (int i = 0; i < inputLayer.size() - 1; i++)
			// don't touch the "bias" input (the last item in the list)
			for (int j = 0; j < layers.get(1).size(); j++)
				connect(0, i, 1, j);

		// wire one layer to the next, starting at layer 1 (the first hidden
		// layer)
	...
	protected void connect(int sourceLayer, int sourceNeuron, int targetLayer,
	        int targetNeuron) {
		new Synapse(layers.get(sourceLayer).get(sourceNeuron), layers.get(
		        targetLayer).get(targetNeuron));
	}
	...
	// 一个RankList对应一个query,在对应的文档集合中组对,构成pairMap
	protected int[][] batchFeedForward(RankList rl) {
		int[][] pairMap = new int[rl.size()][];
		for (int i = 0; i < rl.size(); i++) {
			addInput(rl.get(i));
			propagate(i);// 向网络feed数据，每一个结点保存多个输出值

			int count = 0;// count记录的是与文档i组对的文档数目
			for (int j = 0; j < rl.size(); j++)
				if (rl.get(i).getLabel() > rl.get(j).getLabel())// 组对的条件
					count++;

			pairMap[i] = new int[count];// 为组对文档分配空间
			int k = 0;
			for (int j = 0; j < rl.size(); j++)
	...
	protected void batchBackPropagate(int[][] pairMap, float[][] pairWeight) {
		for (int i = 0; i < pairMap.length; i++) {
			// back-propagate
			PropParameter p = new PropParameter(i, pairMap);// PropParameter包含当前的数据点id及其搭档
			outputLayer.computeDelta(p);// starting at the output layer
			for (int j = layers.size() - 2; j >= 1; j--)
				// back-propagate to the first hidden layer
	...
	protected void saveBestModelOnValidation() {
		for (int i = 0; i < layers.size() - 1; i++)// loop through all layers
		{
			...
			l.clear();
			for (int j = 0; j < layers.get(i).size(); j++)// loop through all
														  // neurons on in the
														  // current layer
			{
				Neuron neuron = layers.get(i).get(j);
				for (int k = 0; k < neuron.getOutLinks().size(); k++)
					// loop through all out links (synapses) of the current
					// neuron
					l.add(neuron.getOutLinks().get(k).getWeight());
			}
		}
	}

	// 在训练结束以后，如果希望使用在校验集上表现最好的模型，则将保存的权重重新赋予网络
	protected void restoreBestModelOnValidation() {
		try {
			for (int i = 0; i < layers.size() - 1; i++)// loop through all
													   // layers
			{
				List<Double> l = bestModelOnValidation.get(i);
				int c = 0;
				for (int j = 0; j < layers.get(i).size(); j++)// loop through
															  // all neurons on
															  // in the current
															  // layer
				{
			...
			System.out
			        .println("Error in NeuralNetwork.restoreBestModelOnValidation(): "
			                + ex.toString());
		}
	}
			...
			for (int k = 0; k < rl.size() - 1; k++) {
				...
				for (int l = k + 1; l < rl.size(); l++) {
					if (rl.get(k).getLabel() > rl.get(l).getLabel()) {
						...
						if (o1 < o2)
							misorderedPairs++;// 错误排序的对
					}
				}
			}
		}
		error = SimpleMath.round(error / totalPairs, 4);// 平均损失

		// if(error > lastError)
		// Neuron.learningRate *= 0.5;
		lastError = error;
	}
	...
	public void learn() {
		...
		PRINTLN("--------------------------------------------------");
		PRINTLN(new int[] { 7, 14, 9, 9 }, new String[] { "#epoch",
		        "% mis-ordered", scorer.name() + "-T", scorer.name() + "-V" });
		...
		for (int iter = 1; iter <= nIteration; iter++) {
			for (int j = 0; j < samples.size(); j++) {
				// 一个RankList对应一个query，以及与之关联的一组docs
				RankList rl = samples.get(j);
				// RankList rl =
				// internalReorder(samples.get(j));//函数internalReorder打酱油的

				int[][] pairMap = batchFeedForward(rl);// 向网络批量feed数据的同时，获取组对结果pairMap
				float[][] pairWeight = computePairWeight(pairMap, rl);// TODO:主要是用于LambdaRank算法中，对于算法RankNet木有作用
				batchBackPropagate(pairMap, pairWeight);// 批量后向传播-计算delta值并更新权值
				clearNeuronOutputs();
			}

			// 针对所有的训练样本对，完成一次训练
			// printWeightVector();
			scoreOnTrainingData = scorer.score(rank(samples));// 首先对所有的基于相同检索词的样本评分，并对样本按照分值降序排列，计算各个检索词下的文档排名性能的平均分值
			estimateLoss();// 使用cross entropy计算平均误差,并计量错误排序的对
			PRINT(new int[] { 7, 14 },
			        new String[] {
			                iter + "",
			                SimpleMath.round(((double) misorderedPairs)
			                        / totalPairs, 4)
			                        + "" });
			...
			if (iter % 1 == 0) {
				PRINT(new int[] { 9 },
				        new String[] { SimpleMath.round(scoreOnTrainingData, 4)
				                + "" });// 测试集上的平均分值
				if (validationSamples != null) {
					double score = scorer.score(rank(validationSamples));// 校验集上的平均分值
					if (score > bestScoreOnValidationData)// 在校验集上表现最好的迭代结果
					{
						bestScoreOnValidationData = score;
						saveBestModelOnValidation();// 保存校验集上表现最好的模型-网络的权重
					}
					PRINT(new int[] { 9 },
		...
		for (int i = 0; i < layers.size() - 1; i++)// loop through all layers
		{
														  ...
														  // current layer
			{
				...
				for (int k = 0; k < n.getOutLinks().size(); k++)
					// loop through all out links (synapses) of the current
					// neuron
					output += n.getOutLinks().get(k).getWeight()
					        + ((k == n.getOutLinks().size() - 1) ? "" : " ");
				output += "\n";
			}
		}
		return output;
	...
	public String model() {
		...
		for (int i = 1; i < layers.size() - 1; i++)
			output += "## Layer " + i + ": " + layers.get(i).size()
			        + " neurons\n";
		...
		try {
			...
			for (; i < l.size(); i++)// loop through all layers
			{
				...
				for (int k = 0; k < n.getOutLinks().size(); k++)
					...
					// neuron
					n.getOutLinks().get(k)
					        .setWeight(Double.parseDouble(s[k + 2]));
			}
		} catch (Exception ex) {
	...
	protected void printNetworkConfig() {
		for (int i = 1; i < layers.size(); i++) {
			...
			for (int j = 0; j < layers.get(i).size(); j++) {
				Neuron n = layers.get(i).get(j);
				System.out.print("Neuron-" + (j + 1) + ": "
				        + n.getInLinks().size() + " inputs\t");
