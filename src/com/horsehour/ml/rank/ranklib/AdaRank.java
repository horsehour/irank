...
import com.horsehour.ml.rank.ranklib.WeakRanker;

/**
 * @author vdang
 * 
 *         This class implements the AdaRank algorithm. Here's the paper: J. Xu
 *         and H. Li. AdaRank: a boosting algorithm for information retrieval.
 *         In Proc. of SIGIR, pages 391-398, 2007.
 */
public class AdaRank extends Ranker {
	...
	//
	// public static int maxSelCount = 5;//the max. number of times a feature
	// can be selected consecutively before being removed
	//
	// protected Hashtable<Integer, Integer> usedFeatures = new
	...
	// {
	// //度量预测精度-已经按照预测分值降序排列
	// double t = scorer.score(wr.rank(samples.get(j)));
	// t *= sweight[j];
	// s += t;
	// }
	// if(bestScore < s)
	...
	// if(withEnqueue)
	// {
	// if(bestWR.getFID() == lastFeature)//this feature is selected twice in a
	// row
	// {
	// //enqueue this feature
	// featureQueue.add(lastFeature);
	// //roll back the previous weak ranker since it is based on this
	// "too strong" feature
	...
	// PRINTLN(new int[]{8, 9, 9, 9}, new String[]{bestWR.getFID()+"", "", "",
	// "ROLLBACK"});
	// continue;
	// }
	// else
	...
	// trainedScore += tmp;
	// }
	// trainedScore /= samples.size();//训练集上平均表现
	...
	// lastFeatureConsecutiveCount = 0;//性能发生改变，特征连续被选中的次数置为0
	...
	// {
	// lastFeatureConsecutiveCount++;//性能未发生改变，特征连续被选中的次数+1
	...
	// }
	//
	// PRINT(new int[]{8, 9, }, new String[]{bestWR.getFID()+"",
	// SimpleMath.round(trainedScore, 4)+""});
	...
	// }
	// PRINT(new int[]{9, 9}, new String[]{SimpleMath.round(scoreOnValidation,
	// 4)+"", status});
	// }
	// else
	...
	// for(int i=0;i<sweight.length;i++)//更新样本概率分布-基于原始分布乘积的形式
	...
	// PRINTLN("--------------------------------------------------------");
	// PRINTLN(new int[]{7, 8, 9, 9, 9}, new String[]{"#iter", "Sel. F.",
	// scorer.name()+"-T", scorer.name()+"-V", "Status"});
	// PRINTLN("--------------------------------------------------------");
	//
	// if(trainWithEnqueue)
	// {
	...
	public static int maxSelCount = 5;// the max. number of times a feature can
									  // be selected consecutively before being
									  // removed
	...
	private int learn(int startIter, boolean withEnqueue) {
		int t = startIter;
		for (; t <= nIteration; t++) {
			WeakRanker bestWR = learnWeakRanker();
			if (bestWR == null)
			...
			if (withEnqueue) {
				if (bestWR.getFID() == lastFeature) {// this feature is selected
													 // twice in a row
					// enqueue this feature
					featureQueue.add(lastFeature);
					// roll back the previous weak ranker since it is based on
					// this "too strong" feature
					rankers.remove(rankers.size() - 1);
					rweight.remove(rweight.size() - 1);
					...
					lastFeature = bestWR.getFID();
					// save the distribution of samples' weight in case we need
					// to rollback
					copy(sweight, backupSampleWeight);
					backupTrainScore = lastTrainedScore;
				}
			...
			if (!withEnqueue) {
				if (trainedScore != lastTrainedScore) {
					...
					if (lastFeature == bestWR.getFID()) {
						...
						if (lastFeatureConsecutiveCount == maxSelCount) {
							lastFeatureConsecutiveCount = 0;
							usedFeatures.put(lastFeature, 1);// removed this
															 // feature from the
															 // pool
						}
			...
			if (t % 1 == 0 && validationSamples != null) {
				double scoreOnValidation = scorer
				        .score(rank(validationSamples));
				if (scoreOnValidation > bestScoreOnValidationData) {
					bestScoreOnValidationData = scoreOnValidation;
					updateBestModelOnValidation();
				}
			...
			lastTrainedScore = trainedScore;
			for (int i = 0; i < sweight.length; i++)
				sweight[i] = Math.exp(-alpha_t
				        * scorer.score(rank(samples.get(i))))
				        / total;
		}
		return t;
	}
	...
	public double eval(DataPoint p) {
		...
		for (int j = 0; j < rankers.size(); j++)
			score += rweight.get(j)
			        * p.getFeatureValue(rankers.get(j).getFID());
		return score;
	}
	...
	public String toString() {
		...
		for (int i = 0; i < rankers.size(); i++)
			output += rankers.get(i).getFID() + ":" + rweight.get(i)
			        + ((i == rankers.size() - 1) ? "" : " ");
		return output;
	}
	...
	public String model() {
		...
		output += "## Train with enqueue: "
		        + ((trainWithEnqueue) ? "Yes" : "No") + "\n";
