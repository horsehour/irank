 ...
 * @author vdang
 * 
 *         This class implements LambdaMART. Q. Wu, C.J.C. Burges, K. Svore and
 *         J. Gao. Adapting Boosting for Information Retrieval Measures. Journal
 *         of Information Retrieval, 2007.
 */
public class LambdaMART extends Ranker {
	...
	public static int nRoundToStopEarly = 100;// If no performance gain on the
											  // *VALIDATION* data is observed
											  // in #rounds, stop the training
											  // process right away.
	...
	protected int[][] sortedIdx = null;// sorted list of samples in @martSamples
									   // by each feature -- Need initializing
									   // only once
	...
	public static void main(String args[]) {
		...
		Evaluator ev = new Evaluator(RANKER_TYPE.RANDOM_FOREST, METRIC.MAP, 10,
		        METRIC.NDCG, 5);
		...
		// ev.evaluate("data/toy/train.txt", "data/toy/vali.txt",
		// "data/toy/test.txt", "");

		// ev.evaluate("data/Fold1/train.txt", "data/Fold1/test.txt", "");
		// ev.evaluate("data/Fold1/train.txt", "data/Fold1/test.txt", "");
		// ev.evaluate("data/MQ2008/Fold1/train-vali.txt", "",
		// "data/MQ2008/Fold1/test.txt", "");
		// ev.evaluate("data/MQ2008/Fold2/train-vali.txt", "",
		// "data/MQ2008/Fold2/test.txt", "");

		// ev.evaluate("data/MQ2008/Fold1/train.txt",
		// "data/MQ2008/Fold1/vali.txt", "data/MQ2008/Fold1/test.txt", "");
		// ev.evaluate("data/MQ2008/Fold2/train.txt",
		// "data/MQ2008/Fold2/vali.txt", "data/MQ2008/Fold2/test.txt", "");
		// ev.evaluate("data/MQ2008/Fold3/train.txt",
		// "data/MQ2008/Fold3/vali.txt", "data/MQ2008/Fold3/test.txt", "");
		// ev.evaluate("data/MQ2008/Fold4/train.txt",
		// "data/MQ2008/Fold4/vali.txt", "data/MQ2008/Fold4/test.txt", "");
		// ev.evaluate("data/MQ2008/Fold5/train.txt",
		// "data/MQ2008/Fold5/vali.txt", "data/MQ2008/Fold5/test.txt", "");
		// ev.evaluate("data/Fold4/train.txt", "data/Fold4/vali.txt",
		// "data/Fold4/test.txt", "");
		// ev.evaluate("data/Fold5/train.txt", "data/Fold5/vali.txt",
		// "data/Fold5/test.txt", "");

		// ev.evaluate("data/toy/train.txt", "data/toy/test.txt", "");
		// ev.evaluate("data/MQ2007/Fold1/train.txt",
		// "data/MQ2007/Fold1/test.txt", "");
		// ev.evaluate("data/MQ2007/Fold1/train.txt", 0.8,
		// "data/MQ2007/Fold1/test.txt", "");

		// ev.evaluate("data/MQ2007/Fold1/train.txt",
		// "data/MQ2007/Fold1/vali.txt", "data/MQ2007/Fold1/test.txt", "");
		ev.evaluate("f:/research/data/MQ2008/Fold2/train.txt",
		        "f:/research/data/MQ2008/Fold2/vali.txt",
		        "f:/research/data/MQ2008/Fold2/test.txt", "");
		// ev.evaluate("data/MQ2007/Fold3/train.txt",
		// "data/MQ2007/Fold3/vali.txt", "data/MQ2007/Fold3/test.txt", "");
		// ev.evaluate("data/MQ2007/Fold4/train.txt",
		// "data/MQ2007/Fold4/vali.txt", "data/MQ2007/Fold4/test.txt", "");
		// ev.evaluate("data/MQ2007/Fold5/train.txt",
		// "data/MQ2007/Fold5/vali.txt", "data/MQ2007/Fold5/test.txt", "");
		// ev.evaluate("data/MSLR-WEB10K/Fold1/train.txt",
		// "data/MSLR-WEB10K/Fold1/vali.txt", "data/MSLR-WEB10K/Fold1/test.txt",
		// "");
	...
	public void init() {
		...
		// sort (MART) samples by each feature so that we can quickly retrieve a
		// sorted list of samples by any feature later on.
		...
		else// multi-thread
		{
			...
			for (int i = 0; i < partition.length - 1; i++)
				p.execute(new SortWorker(this, partition[i],
				        partition[i + 1] - 1));
			p.await();
		}

		// Create a table of candidate thresholds (for each feature). Later on,
		// we will select the best tree split from these candidates
		thresholds = new float[features.length][];
		for (int f = 0; f < features.length; f++) {
			// For this feature, keep track of the list of unique values and the
			// max/min
			List<Float> values = new ArrayList<Float>();
			float fmax = Float.NEGATIVE_INFINITY;
			float fmin = Float.MAX_VALUE;
			for (int i = 0; i < martSamples.length; i++) {
				int k = sortedIdx[f][i];// get samples sorted with respect to
										// this feature
				...
				while (j < martSamples.length) {
					if (martSamples[sortedIdx[f][j]]
					        .getFeatureValue(features[f]) > fv)
						break;
					j++;
				}
				i = j - 1;// [i, j] gives the range of samples with the same
						  // feature value
			}
		...
		if (validationSamples != null) {
			...
			for (int i = 0; i < validationSamples.size(); i++) {
				modelScoresOnValidation[i] = new float[validationSamples.get(i)
				        .size()];
				Arrays.fill(modelScoresOnValidation[i], 0);
			}
		}

		// compute the feature histogram (this is used to speed up the procedure
		// of finding the best tree split later on)
		hist = new FeatureHistogram();
		hist.construct(martSamples, pseudoResponses, sortedIdx, features,
		        thresholds);
	...
	public void learn() {
		...
		PRINTLN("---------------------------------");
		PRINTLN(new int[] { 7, 9, 9 }, new String[] { "#iter",
		        scorer.name() + "-T", scorer.name() + "-V" });
		PRINTLN("---------------------------------");
		...
		for (int m = 0; m < nTrees; m++) {
			...
			// update the histogram with these training labels (the feature
			// histogram will be used to find the best tree split)
			hist.update(pseudoResponses);

			// Fit a regression tree
			RegressionTree rt = new RegressionTree(nTreeLeaves, martSamples,
			        pseudoResponses, hist, minLeafSupport);
			rt.fit();

			// Add this tree to the ensemble (our model)
			ensemble.add(rt, learningRate);

			// update the outputs of the tree (with gamma computed using the
			...
			// **** NOTE ****
			// The above function to evaluate the current model on the training
			// data is equivalent to a single call:
			//
			// scoreOnTrainingData = scorer.score(rank(samples);
			//
			// However, this function is more efficient since it uses the cached
			// outputs of the model (as opposed to re-evaluate the model
			// on the entire training set).

			PRINT(new int[] { 9 },
			        new String[] { SimpleMath.round(scoreOnTrainingData, 4)
			                + "" });
			...
			if (validationSamples != null) {
				...
				for (int i = 0; i < modelScoresOnValidation.length; i++)
					for (int j = 0; j < modelScoresOnValidation[i].length; j++)
						modelScoresOnValidation[i][j] += learningRate
						        * rt.eval(validationSamples.get(i).get(j));

				// again, equivalent to
				// scoreOnValidation=scorer.score(rank(validationSamples)), but
				// more efficient since we use the cached models' outputs
				double score = computeModelScoreOnValidation();
				PRINT(new int[] { 9 },
				        new String[] { SimpleMath.round(score, 4) + "" });
		...
		PRINTLN(scorer.name() + " on training data: "
		        + SimpleMath.round(scoreOnTrainingData, 4));
		if (validationSamples != null) {
			bestScoreOnValidationData = scorer.score(rank(validationSamples));
			PRINTLN(scorer.name() + " on validation data: "
			        + SimpleMath.round(bestScoreOnValidationData, 4));
		}
		PRINTLN("---------------------------------");
	}
	...
	public void printParameters() {
		...
		PRINTLN("Stop early: " + nRoundToStopEarly
		        + " rounds without performance gain on validation data");
	}
	...
	protected void computePseudoResponses() {
		...
		// divide the entire dataset into chunks of equal size for each worker
		// thread
		...
		for (int i = 0; i < p.size(); i++) {
			...
			// execute the worker
			LambdaComputationWorker wk = new LambdaComputationWorker(this,
			        start, end, current);
			workers.add(wk);// keep it so we can get back results from it later
							// on
			p.execute(wk);
			...
			for (int j = 0; j < r.size(); j++) {
				...
				for (int k = 0; k < r.size(); k++) {
					...
					if (p1.getLabel() > p2.getLabel()) {
						double rho = 1.0 / (1 + Math.exp(modelScores[current
						        + j]
						        - modelScores[current + k]));
	...
	/**
	 * This function is equivalent to the inherited function rank(...), but it
	 * uses the cached model's outputs instead of computing them from scratch.
	 * 
	 * @param rankListIndex
	 * @param current
	 * @return
	...
	/**
	 * Compute the change (in whatever specified metric) for swapping each pair
	 * of documents in a rank list.
	 * 
	 * @param rankListIndex
	 * @param current
	 * @return
	...
	class LambdaComputationWorker implements Runnable {
		...
		LambdaComputationWorker(LambdaMART ranker, int rlStart, int rlEnd,
		        int martStart) {
			this.ranker = ranker;
