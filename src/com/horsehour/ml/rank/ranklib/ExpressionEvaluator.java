...
public class ExpressionEvaluator {
	...
	private static String[] functions = new String[] { "log", "ln", "log2",
	        "exp", "sqrt", "neg" };
			...
			if (token.compareTo("(") == 0)
				...
				if (foundOpen == false) {
					System.out.println("Error: Invalid expression: \""
					        + expression + "\". Parentheses mismatched.");
					System.exit(1);
				}
			} else if (isOperator(token)) {
				if (lastReadToken.compareTo("(") == 0
				        || isOperator(lastReadToken))// @token is a unary
													 // opeartor (e.g. +2, -3)
				{
	...
	public double eval(String expression) {
		...
		try {
			...
			System.out
			        .println("Unknown error in ExpressionEvaluator::eval() with \""
			                + expression + "\"");
